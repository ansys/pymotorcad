...
# %%
# This script is designed to be run from a Motor-CAD model based on template "e4a", a 48 slot,
...
# %%
# If no Motor-CAD file is open, the e4a template will be loaded and the geometry will be adjusted as
# described above.
...
# Import ``Coordinate``, ``Arc``, ``Line`` ``rt_to_xy``
...
# the e4a IPM motor template is loaded , the geometry changes described above are applied and the
...
# %%
# Use the ``set_default_parameter()`` method to set the required ``bez_curve_projection``,
...
# %%
# The adaptive parameters will be used to define the curved rotor pocket geometry with a Bezier
...
# %%
# Get properties of the Magnet edge that will be used to define the new Rotor Pocket geometry
...
# %%
# x-y Points that will be used to draw the new Rotor Pocket - relative to a vertical magnet edge
...
# %%
# Mirror the first rotor pocket region on the other half of the rotor
# Creates a new region mirroredRegion
...
# %%
# Replace the entities in the second Rotor Pocket with those from mirroredRegion
