...
#
# Motor-CAD creates a thermal model of the motor using a Lumped Parameter Thermal Network (LPTN), which allows the machine temperatures to 
# be calculated. This LPTN thermal model can be formulated as a set of thermal matrices. Motor-CAD provides the capability to export these, 
# allowing the thermal model to be imported into other engineering software packages. In general, the exported model is accurate at a single
# speed, housing temperature, and airgap temperature ("operating point").
#
# In Twin Builder 2024 R2 and later, a *Motor-CAD ROM* component can be created which extends upon this idea, allowing for the creation of a 
# Motor-CAD thermal model that is valid at a range of operating points. It does this by utilizing data exported from several discrete operating 
# points to generate the component, and then automatically interpolating between these during a solve. The component also solves the coolant 
...
#
# The *Motor-CAD ROM* component is quick to set up and provides a significantly more accurate model compared to the single operating point export.
# It has a user friendly interface with losses and RPM as input pins, and component temperatures as output pins. Once generated, the 
# component is standalone (does not require Motor-CAD), thus allowing it to be shared/used in alternate systems whilst obscuring the 
# underlying Motor-CAD geometry.

# %%
# Data required to generate a *Motor-CAD ROM* component
# ----------------------------------------
# To generate the component, within Ansys Electronics Desktop, go to the menu bar and select **Twin Builder** > **Add Component** > **Add Motor-CAD ROM Component...**.
# This will present the following window: 
#
# .. image:: ../../images/Thermal_Twinbuilder_GenerateROM_Blank.png
#
# The **Input Files** must point to the folder which contains the Motor-CAD data at the appropriate operating points of interest, formatted in 
# the appropriate manner. The script below is an example showing how this can be done.  
#
# .. note::
#    This script is an example of how the data needed to generate a *Motor-CAD ROM* component can be obtained. For 
#    details on how the resulting *Motor-CAD ROM* component can be used, please consult the Twin Builder Help Manual.
#
# .. note:: 
...
# 4. For each desired speed, the thermal model is solved and thermal matrices exported and saved to the ``dpx`` folders
...
class MotorCADTwinModel:
    ...
    # the Motor-CAD loss names (for display in Twinbuilder), and the corresponding Motor-CAD parameter names.  
    coolingSystemNodeGroups = ['End Space', 'Ventilated', 'Housing Water Jacket',
                               'Shaft Spiral Groove', 'Wet Rotor', 'Spray Cooling',
                               'Rotor Water Jacket', 'Slot Water Jacket', 'Heat Exchanger']

    lossNames = ['Loss_Armature_Copper_dc',
    ...
    lossParameters = ['Armature_Copper_Loss_@Ref_Speed',
                      ...
                      # Stator_Iron_Loss_@Ref_Speed_[Tooth_Tip] is not included as this is an unused Motor-CAD parameter
    ...
    def generateTwinData(self, parameters: dict, housingAmbientTemperatures=None, airgapTemperatures=None):        

        self.updateMotfile()

        # calculate self.nodeNames, self.nodeNumbers, self.nodeGroupings, self.nodeNumbers_fluid, and self.nodeNumbers_fluidInlet
        self.getNodeData()
    ...
    # Helper functions to parse the exported Motor-CAD matrices (``.cmf``, ``.nmf``, ``.pmf``, ``.rmf`` and .``.tmf``)
    ...
    def getNodeData(self):
        ...
        self.nodeNumbers_fluid = [nodeNumber for (index, nodeNumber) in enumerate(self.nodeNumbers) if self.nodeGroupings[index] in coolingSystemsPresent]


    # Function that determines the nodes used for the cooling system and their connections. The resulting data is required
    # by Twin Builder to correctly model the fluid flow
    def generateCoolingSystemNetwork(self):
        ...
        if len(self.nodeNumbers_fluid) == 0:
            ...
            for node in self.nodeNumbers_fluid:
                connectedNodes = self.returnConnectedNodes(node, self.nodeNumbers_fluid, resistanceMatrix)
                ...
                while len(next)>0:
                    ...
                    for k in range(0, len(line)):
                        if line[k]>0 and graphNodes[k] not in covered:
                            ...
                            if node != graphNode: 
                                connectedNodesList.append([self.unbracket(self.nodeNames[self.nodeNumbers.index(node)]),
                                                           self.unbracket(self.nodeNames[self.nodeNumbers.index(graphNodes[k])])])
            ...
            with open(os.path.join(self.outputDirectory, 'CoolingSystems.csv'), 'w') as outfile:
                ...
                for connectedNodesList in connectedNodesLists:
                    outfile.write('inlet : ' + str(inletNodes[k]) + ' - ' + str(
                        self.nodeNames[self.nodeNumbers.index(inletNodes[k])]) + '\n')
    ...
    # Function that extracts the per-node loss distribution for each loss type, allowing the user to specify
    # a loss value using a name (such as Armature Copper Loss) and have Twin Builder automatically distribute
    # this amongst appropriate nodes. 
    def generateLossDistribution(self):
        ...
        for lossIndex in range(numLossParameters):
            print("power distribution "+str(lossIndex+1)+"/"+str(numLossParameters)+" : "+ self.lossNames[lossIndex])
    ...
    # Function that determines the Housing to Ambient resistances at different housing temperatures, the results of which
    # are used by Twin Builder to take into account external Natural Convection cooling
    # The input parameter is a dict, format e.g.:
    # {tAmbient1:[tHousing1, ..., tHousingx], tAmbient2:[tHousing1,..., tHousingy], tAmbient3:[tHousing1, ..., tHousingx]}
    def generateHousingTempDependency(self, housingAmbientTemperatures):
        housingNodes = [nodeNumber for (index, nodeNumber) in enumerate(self.nodeNumbers) if self.nodeGroupings[index] == "Housing"]
        ...
        for ambientTemperature, fixedHousingTemperatures in housingAmbientTemperatures.items():
            ...
            file_content, header_line = self.computeMatricesHousingTemps(housingNodes, ambientTemperature, fixedHousingTemperatures)

            exportDirectory = os.path.join(self.outputDirectory, 'HousingTempDependency')
            if not os.path.isdir(exportDirectory):
                os.makedirs(os.path.join(exportDirectory))

            with open(os.path.join(exportDirectory, 'Housing_Temp' + str(fileInd) + '.csv'), 'w') as fout:
    ...
    def computeMatricesHousingTemps(self, housingNodes, ambientTemperature, fixedHousingTemperatures):
        exportDirectory = os.path.join(self.outputDirectory, 'tmp')
    ...
    # Function that determines the Stator to Rotor airgap resistance at different housing temperatures, the results of which
    # are used by Twin Builder to take into account the temperature dependent nature of this resistance
    ...
    def getAirgapNodes(self):        
        ...
        if tVent or sVent:
            ...
            if statorCoolingOnly == False:
                warnings.warn('Temperature dependent airgap not supported for ventilated cooling with airgap flow')
...
# Example use case
# ----------------
# Below is an example of how the above ``MotorCADTwinModel`` class can be used. The .mot file that will be used is the 
# ``e8_eMobility`` template. 

# %%
# The ``generateTwinData`` method accepts as an optional parameter a dictionary of Housing and Ambient temperatures to be 
# investigated. This can be provided if Natural Convection cooling of the housing should be modelled in the Twin Builder 
# *Motor-CAD ROM* component. For this example, a function has been defined to return this dictionary. As can be seen 
# in the code comments, more data points are calculated when the housing and ambient temperatures are close together, 
# as this is where the natural convection heat transfer coefficients vary the most.
...
# %%
# Choose the speed points that the model should be solved at. The generated *Motor-CAD ROM* component will interpolate between these,
# so it is important to cover the complete speed range with the appropriate sampling in order to maintain accuracy. Three points have 
# been chosen here to reduce calculation time, but in real use it is recommended that this be greater.
rpms = [200, 500, 1000]

# %%
# Specify the airgap temperatures to investigate, in order for the temperature dependent nature of the airgap heat transfer to 
# be included in the *Motor-CAD ROM* component. The generated *Motor-CAD ROM* component will interpolate between these,
# so it is important to cover the complete expected airgap temperature range with the appropriate sampling in order to maintain accuracy.
# This parameter can be set to ``None`` should this not be required.
airgapTemps = [40, 50, 65]

# %%
# Specify the housing and ambient temperatures to investigate, in order for the natural convection cooling of the housing to be
# be included in the *Motor-CAD ROM* component. The generated *Motor-CAD ROM* component will interpolate between these, so it is important 
# to cover the complete expected housing and ambient temperature range with the appropriate sampling in order to maintain accuracy.
# This parameter can be set to ``None`` should this not be required.
housingAmbientTemps = temperaturesHousingAmbient()

# %%
# Create a ``MotorCADTwinModel`` object, passing as arguments the path to the input .mot file as well as the directory to which the 
# results should be saved.
MotorCADTwin = MotorCADTwinModel(inputMotFilePath, 
                                 outputDir)

# %%
# Finally, generate the required data. This function will write the data to the directory specified above. 
# The identified cooling system node flow path is automatically plotted.
MotorCADTwin.generateTwinData(parameters={'rpm':rpms}, 
...
# ----------------------------------------
# To generate the component, within Ansys Electronics Desktop, go to the menu bar and select **Twin Builder** > **Add Component** > 
# **Add Motor-CAD ROM Component...**. Under **Input Files**, press the ``...`` icon and choose the ``outputDir`` as specified in the 
# previous step. Then press the **Generate** button.
#
# .. image:: ../../images/Thermal_Twinbuilder_GenerateROM_Filled.png
#
# Should the generation be successful, the **Log** will indicate that the SML model has been written and the **Select Interfaces** table
# will be populated.
#
